...
type Screen = 'landing' | 'student-login' | 'admin-login' | 'student-dashboard' | 'admin-dashboard' | 'first-setup';
...
function FirstSetupScreen({ onCreated, onBack }: { onCreated: (a: Admin) => void; onBack: () => void }) {
  ...
  const handleCreate = () => {
    if (!nick.trim()) { setError('Введите псевдоним'); return; }
    if (pass.length < 4) { setError('Пароль — минимум 4 символа'); return; }
    if (pass !== pass2) { setError('Пароли не совпадают'); return; }
    setLoading(true);
    setTimeout(() => {
      onCreated({ id: Date.now(), nickname: nick.trim(), password: pass, avatar: '🎓' });
    }, 600);
  };
      ...
      <div className="relative z-10 w-full max-w-sm">
        ...
        <div className="opacity-0 animate-fade-in-up delay-200 glass rounded-3xl p-8" style={{ animationFillMode: 'forwards' }}>
          <div className="text-center mb-6">
            ...
            <h2 className="font-golos font-black text-2xl mb-1">Первый запуск</h2>
            <p className="font-golos text-sm text-muted-foreground">Создайте аккаунт администратора.<br/>После этого кнопка исчезнет навсегда.</p>
          </div>

          <div className="p-3 rounded-xl mb-5 flex items-start gap-2" style={{ background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.25)' }}>
            <span className="text-green-400 mt-0.5">✓</span>
            <p className="font-golos text-xs text-green-400">Через этот аккаунт вы сможете добавить всех студентов и других администраторов</p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-golos font-semibold text-muted-foreground mb-2 uppercase tracking-wider">Повторите пароль</label>
            ...
            <button onClick={handleCreate} disabled={loading || !nick || !pass || !pass2}
...
function LandingScreen({ onRole, onFirstSetup, isFirstLaunch }: {
  onRole: (r: 'student' | 'admin') => void;
  onFirstSetup: () => void;
        ...
        <div className="opacity-0 animate-fade-in-up delay-600 space-y-3" style={{ animationFillMode: 'forwards' }}>
          ...
          {isFirstLaunch && (
            <button
              ...
              className="w-full py-3 px-6 rounded-2xl font-golos font-semibold text-sm transition-all duration-300 hover:scale-105 active:scale-95"
              style={{ background: 'rgba(16,185,129,0.12)', border: '1px solid rgba(16,185,129,0.3)', color: '#10b981' }}
            >
              <span className="flex items-center justify-center gap-2">
                <Icon name="KeyRound" size={16} />
                Первый запуск · Создать аккаунт
              </span>
            </button>
          )}
        </div>
...
export default function Index() {
  ...
  const handleFirstSetupCreated = (newAdmin: Admin) => {
    const updated = [newAdmin];
    setAdmins(updated);
  ...
  return (
    <>
      {screen === 'landing' && (
        <LandingScreen
          onRole={handleRole}
          onFirstSetup={() => setScreen('first-setup')}
          isFirstLaunch={admins.length === 0}
        />
      )}

      {screen === 'first-setup' && (
        <FirstSetupScreen
          onCreated={handleFirstSetupCreated}
